...
from intercodeTM import InterCode
from interccodeP import generar_codigo
...
def run_command(root, mensaje, texto, frame_lexico, pantalla_errores, frame_sintactico, frame_semantico, frame_symtab, frame_pcode):
    global ruta
    ...
    if ruta != "":
        ...
        for child in frame_symtab.winfo_children():
            child.destroy()
        for child in frame_pcode.winfo_children():
            child.destroy()
       ...
       # Ajustar las columnas para llenar el espacio
        ...
        if errors:
            ...
            run_syntax_analysis(mensaje, tokens_to_text(tokens), frame_sintactico, pantalla_errores, tokens, frame_semantico, frame_symtab, frame_pcode, input_text)

    else:
        mensaje.set("No hay archivo abierto para ejecutar análisis léxico.")

def run_syntax_analysis(mensaje, texto, frame_sintactico, pantalla_errores, tokens, frame_semantico, frame_symtab, frame_pcode, input_text):
    ...
    if tokens:
        ...
        if result:
            ...
            run_semantic_analysis(result, mensaje, frame_semantico, pantalla_errores, frame_symtab, tokens, frame_pcode, input_text)
        else:
...
def run_semantic_analysis(result, mensaje, frame_semantico, pantalla_errores, frame_symtab, tokens, frame_pcode, input_text):
    ...
    run_symtab(mensaje, frame_symtab, analyzer.get_symbol_table(), tokens, frame_pcode, result)

    # After completing semantic analysis, pass the AST and symbol table to run_pcode
    run_pcode(mensaje, frame_pcode, input_text, result)
...
def run_symtab(mensaje, frame_symtab, symtab_items, tokens, frame_pcode, ast):
...
def run_pcode(mensaje, frame_pcode, input_text, ast=None):
    print("AST (result):")
    print(ast)
    recorrer_arbol(ast)

    #intermediate_code = generate_intermediate_code(input_text)
    if ast is None:
        mensaje.set("No se puede generar código intermedio. AST faltante.")
        return
    
    mensaje.set("Generando código intermedio...")

     # --- Call generar_codigo() to generate intermediate code ---
    try:
        intermediate_code = generar_codigo(ast)  # Call the function using the AST
        mensaje.set("Código intermedio generado correctamente.")
    except Exception as e:
        mensaje.set(f"Error al generar código intermedio: {e}")
        return

    # --- Display the intermediate code in frame_pcode ---
    # Clear the frame_pcode before displaying the new code
    for child in frame_pcode.winfo_children():
        child.destroy()

    # Create a text widget to display the intermediate code
    text_widget = tk.Text(frame_pcode, wrap='word')
    text_widget.insert('1.0', intermediate_code)  # Insert the generated code
    text_widget.pack(fill='both', expand=True)

    # Optionally disable editing if you want it to be read-only
    text_widget.config(state='disabled')

def recorrer_arbol(node): 
    if node is not None: 
        print(f"Node Type: {node.type}, Leaf: {node.leaf}, Level: {node.level}") 
        for child in node.children: recorrer_arbol(child) 

def arbol_a_cadena(node, nivel=0): 
    result = "" 
    if node is not None: 
        indent = " " * nivel 
        result += f"{indent}Node Type: {node.type}, Leaf: {node.leaf}, Level: {node.level}\n" 
        for child in node.children: result += arbol_a_cadena(child, nivel + 1) 
        return
